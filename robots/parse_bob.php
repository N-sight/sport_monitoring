...
function page_bob ($line)
{
    ...
    $allnews[0] = parse_comments($blocknews,$line);
    ...
    if (isset($blocknews[6])) {
        if (contains($blocknews[6], 'button'))// значит комменты еще есть
        {
            ...
            do {
                $n++;
                $ext[$n] = get_add_bob($n, $line['url']);
                if (!$ext[$n]) {
                    writeln('stuck!');
                    $ext[$n] = get_add_bob($n, $line['url']);
                }
                $allnews[$n] = parse_comments($ext[$n]['html'], $line, false);
...
function parse_comments($arr,$line,$convert = true)
{
    ...
    for ($i=0; $i<count($arr); $i++)
    {
        if (strpos($arr [$i], 'js Comments_Element comment-post') !== false) //
        {
            ...
            $result[$k]['time'] = patch_time_parse_bob(trim(strip_tags($time)));
...
function patch_time_parse_bob ($str)
{
...
function get_add_bob ($n,$url)
{
    ...
    if (strlen($msec) !== 4)
    {
        ...
        {
            case 3: $msec.='0';break;
            case 2: $msec.='00';break;
            case 1: $msec.='000';break;
        }
    }
    $utime = $mtime[1].$msec;
