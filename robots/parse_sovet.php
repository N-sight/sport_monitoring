...
function page_sovet ($line)
{
    ...
    if (mb_strpos($id, 'adfox') != true)
    {
        ...
        if (!$data) {
            die('ошибка при получении контента с '.$jsurl);}
        ...
        for ($i = 0; $i < count($rows); $i++) {
            if (strpos($rows[$i], '\'\'+') !== false) {
                ...
                $comm[$n]['time'] = patch_time( mb_substr($rows[$i + 1], 3, mb_strpos($rows[$i + 1], '\', ') - 3), $line['date'] );
...
function patch_time ($str,$mon_day)
{
